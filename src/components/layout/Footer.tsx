...
import { footerT } from '@/translations';

export default function Footer() {
  const { lang } = useLang();
  const tr = footerT[lang];
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="md:col-span-1">
            <Link href="/" className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold-muted))' }}>
            ...
            <p className="text-sm leading-relaxed mt-4" style={{ color: 'var(--mist)' }}>{tr.tagline}</p>
            <div className="flex gap-4 mt-6">
              {[Share2, Globe, LinkIcon].map((Icon, i) => (
                <a key={i} href="#" className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200"
          ...
          {/* Programs */}
          <div>
            <h4 className="text-xs tracking-[0.2em] uppercase mb-5 font-ui" style={{ color: 'var(--gold)' }}>{tr.programs}</h4>
            <ul className="space-y-3">
              {tr.programList.map(p => (
                <li key={p}>
          ...
          {/* Quick Links */}
          <div>
            <h4 className="text-xs tracking-[0.2em] uppercase mb-5 font-ui" style={{ color: 'var(--gold)' }}>{tr.quickLinks}</h4>
            <ul className="space-y-3">
              {tr.links.map(l => (
                <li key={l.href}>
          ...
          {/* Contact */}
          <div>
            <h4 className="text-xs tracking-[0.2em] uppercase mb-5 font-ui" style={{ color: 'var(--gold)' }}>{tr.contact}</h4>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs font-ui" style={{ color: 'var(--mist)' }}>
            © {new Date().getFullYear()} TOPmusic. {tr.rights}
          </p>
